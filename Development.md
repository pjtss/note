...
### [2026-05-23] 메모지 모던 파스텔톤 컬러 리팩토링 및 가독성 하위 호환성 확보 (최신)
- **작업 내용**:
  1. **요구사항 수용**:
     - 기존 오션 블루 계열의 메모지 테마 중 너무 짙거나 어두워 가독성을 해치고 시각적 통일감을 저해하는 색상들을 부드럽고 은은한 모던 파스텔톤으로 완전 개편 요청 반영.
  2. **모던 파스텔톤 9종 큐레이션 개편**:
     - 기존의 `#4ea8de` (오션블루 - 진함), `#0077b6` (딥블루 - 매우 진함), `#1d3557` (마린네이비 - 매우 진함) 등 어두운 색상을 완전히 탈피.
  ...
  3. **가독성 하위 호환성(checkIfDarkColor) 헬퍼 도입**:
     - 데이터베이스나 로컬 스토리지에 기 저장된 기존 구형 다크 색상 메모들의 글자 및 보더 가독성을 안전하게 보호하기 위해 `checkIfDarkColor` 헬퍼 함수를 신설.
     - 구형 진한 색상 메모가 렌더링될 때는 텍스트와 테두리가 자동으로 화이트 톤으로 최적화 매칭되도록 하위 호환성을 완벽 구현.
  4. **기본 컬러 초기화 값 통일**:
     - 메모 작성 완료 또는 편집 취소 시 메모 기본 할당 컬러가 `#ffd166` (다소 짙은 옐로우)로 리셋되던 불일치를 해결하고, 9종 파스텔의 대표 라이트 옐로우인 `#fffbeb` (밀크바닐라)로 통합하여 완성도 상승.
  5. **100% 테스트 무결성 수호**:
     - Jest 단위 테스트 스위트 100% 올패스 및 Next.js 프로덕션 컴파일 0 Error 빌드 통과.
